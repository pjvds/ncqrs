...
namespace Ncqrs.Commanding.CommandExecution.Mapping
{
                                                                                                    ...
                                                                                                    where TAggregateRoot : AggregateRoot
    {
        ...
        public DirectMethodCommandExecutor(Func<TCommand, Guid> getAggId, Func<TCommand, TAggregateRoot> action) : this(getAggId, action, NcqrsEnvironment.Get<IUnitOfWorkFactory>())
        {
        }

        public DirectMethodCommandExecutor(Func<TCommand, Guid> getAggId, Func<TCommand, TAggregateRoot> action, IUnitOfWorkFactory uowFactory)
        {
            _getAggId = getAggId;
            _action = action;
