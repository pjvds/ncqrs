...
namespace Ncqrs.Eventing.Sourcing
{
    public abstract class EventSource : IEventSource
    {
        ...
        {
            if (evnt.EventSourceId != evnt.UndefinedEventSourceId)
            {
            ...
            if (evnt.EventSequence != evnt.UndefinedEventSequence)
            {
                // TODO: Add better exception message.
                var message = String.Format("The {0} event cannot be applied to event source {1} with id {2} " +
                            "since the event already contains a sequence {3} while {4} was expected.",
                            evnt.GetType().FullName, this.GetType().FullName, EventSourceId, evnt.EventSequence, evnt.UndefinedEventSequence);
                throw new InvalidOperationException(message);
            }

            evnt.ApplyEventSourceIdAndSequence(EventSourceId, Version + 1);
